#!/bin/sh
# rue backstop artifact: plan open_mgmt_port on fw-lnx-01 (os linux), instance golden, language sh. Rendered by rue-render; do not edit.
set -u
ROOT='/var/db/rue'
...
if [ -f "$M" ]; then
  ...
  if [ "$skip" -eq 1 ]; then defer 1; else
    strip_region '/etc/nftables.conf' 'rue-mgmt' || { if [ -n "${RUE_NOLOCK:-}" ] || foreign_region '/etc/nftables.conf'; then defer 1; else restore '/var/db/rue/instances/golden/snapshots/1/0' '/etc/nftables.conf'; clobbered 1; fi; }
    rm -f "$M"
  fi
fi
: > "$INST/fired"
