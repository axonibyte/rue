...
Before Phase 0 exits, and again before any public release, one person spends one day trying to name a system that makes reversibility a checked property across heterogeneous operations. If one is found, the claim is narrowed in §1.1 and the delta added to §1.2. Done once, 2026-09-06 (`docs/prior-art.md`): two were found and the claim was narrowed as above. The **name** sweep section 12 requires is in the same file, done 2026-09-12: `rue` is taken on crates.io, PyPI and npm, and by two other programming languages on GitHub, one of which holds `rue` and `rue-lsp` on crates.io; every SDK name is free, and so is `rued`. Recorded for the owner, not decided.
...
`rue drill <file> --host <canary>` reads the digest of every fact the plan's footprint names on the hosts it touches, applies the plan, recants it, reads them again and compares. The drill is an ordinary instance from the engine's side -- it checks, requests, reserves, applies and recants like any other -- so it contends with production work in the ledger rather than stepping around it. Two refusals stand in front of it, both **R0410**: every host the plan touches must carry the role `canary` in the inventory, and the plan must be temporary, since a drill has to undo what it did and a permanent plan is not recanted. A step whose host is bound at runtime is refused the same way: a drill names where it runs before it runs.

The attestation is a journal entry and nothing else -- `DrillAttested{plan, host, instance, restored, facts}`, one line per fact of `<host> <shape> before=<digest> after=<digest>` -- so the chain covers it, and an attestation with a verification of its own would be a second chain to trust. `rue journal verify --attestations` prints them once the chain verifies, and exits non-zero when a chain carries no drill or carries one that attested nothing. `restored` is true only when every fact was read and every digest came back: a fact the engine could not read (R0205) is kept as `unread=<why>`, attests to nothing, and makes the drill say so, as does a footprint that named no fact this engine could read. A drill that reads nothing has proved nothing about the undo it was there to prove.

What schedules it is the operator's own timer -- cron, a systemd timer, launchd -- invoking the verb. `rued` grows no scheduler of its own: rue schedules *backstops* through the site's scheduler binding because they must fire with the engine dead, and a drill is the opposite case, needing the engine alive.
