...
**Acceptance.** T4 and T2 pass; a hook returning `ok:true` with a missing field is refused; a hook missing its deadline is `Silent` and the plan refuses; the same `.rue` text checks identically standalone and embedded.

**Exit criteria.** Acceptance met; hook protocol v1 frozen; tag v0.2.0.

**Not proven.** Any real reactive host; hook round-trip performance under load; NIF embedding (deliberately not built).

**Rediscovery rows seeded.** `hook-missing-field-accepted`, `hook-silent-as-ok`, `embedded-verdict-differs`, `sink-refusal-not-propagated`.
